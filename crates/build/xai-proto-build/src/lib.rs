...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // protoc cannot write its dependency output to `/dev/stdout` on all
        // platforms (notably Windows), so write to a temp file and read it back.
        // Likewise the descriptor output is discarded via the platform null device.
        let null_device = if cfg!(windows) { "NUL" } else { "/dev/null" };

        // Can only process one input file when using --dependency_out=FILE.
        for proto in protos {
            let dep_file = tempfile::NamedTempFile::new()
                .context("failed to create protoc dependency temp file")?;
            ...
            let output =
                fs::read_to_string(dep_path).context("failed to read protoc dependency output")?;
            ...
            for line in iter::once(rem).chain(lines) {
                ...
                // is valid, but we want to have output more deterministic. Normalize
                // path separators so this also matches on Windows.
